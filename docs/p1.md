...
If the thread is the first thread, we will discuss this case in the next section. If not, the `ret` instruction will
unfreeze the page(context), push the saved register, and finally eip will be restored to right after
of `switch_threads()` call.
