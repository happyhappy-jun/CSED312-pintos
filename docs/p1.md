...
- `sema_down()`: P. If the `value` is 0, the caller thread is pushed into the `waiters` by `list_push_back()` and
  blocked
  by `thread_block()`. When the `value` becomes positive, the caller thread decrements the `value` and returns to access
...
- `sema_up()`: V. If `waiters` is not empty (i.e., there are threads that want to access the critical section), the
  ...
  of `waiters`, `sema_up()` increments the `value`.
...
running thread.
The selected running thread does its job and then calls `sema_up()` to make the previous running thread become the
current
...
In this case, the "condition" is "Is Thread B finished its work?". Thread A waits for the condition to become true, and
...
Current implementation of priority scheduling is based on the `ready_list` in FIFO order. It doesn't consider the
priority of the thread. Based on the state diagram of the thread life cycle, a thread is pushed to `ready_list` when
`thread_unblock()` and `thread_yield()` is called. Popping a new thread from `ready_list` is done by `schedule()`.
`thread_unblock()` and `thread_yield()` call `list_push_back()` to push a thread to `ready_list` the end
of `ready_list`. `schedule()` calls `next_thread_to_run()` to pop a thread from `ready_list`. `next_thread_to_run()`
returns the first entry of `ready_list`. In summary, the current implementation of priority scheduling is based on the
naive FIFO queue, which doesn't take account of the priority of the thread.

### Priority Inversion

Priority inversion is a problem that occurs when a high priority thread is waiting for cpu resource that is currently
being used by a low priority thread. A root cause of this is lock that is used to protect a shared resource. Assume
there are three threads, H, M, L with priority H > M > L. If H is waiting for a lock A that is currently held by L,
then M can preempt L. At this point, H is at waiting list of lock A, and L is in ready list. After, M finishes its work,
it gives cpu to L, which is in waiting list. L will run and release the lock A. Then, H will be able to run. Based on
the priority of threads, H should run first. However, due to the priority inversion, a run order of threads becomes
M -> L -> H.

One solution for this is **priority donation**. When a thread is waiting for a lock, it donates its priority to the
holder of the lock. In the above example, H donates its priority to L. Then, L will have the highest priority followed
by L and M

Multiple donation is there is multiple donation occurs. If L hold a lock A and B, and M is will to lock A, then
M will donation its priority to L and go to waiting list of lock A. Next, H comes in then it want to lock B and donate
its priority to L. This is called multiple donation.

Nested donation is a case of recursive donation of priority. A previous example's donation is between M -> L, H -> L.
The nested donation is a case of H -> M -> L. As a precondition, L is holding a lock A, and M is holding a lock B and
requires lock A. If H needs lock B so donate its priority to M only, M still need lock A. However, lock A is held by L,
which doesn't have any donation. To solve this case, weed need recursive donation of priority from H -> M and M -> L.

### Data Structures

To achieve priority scheduling, we need to manage `ready_list` in sorted order. Since `ready_list` is implemented as
`list` in pintos, we can use `list_insert_ordered()` to insert a thread to `ready_list` in sorted order. For this case,
new data strucutres are not needed.

However, due to priority inversion issue, we need some new member in `struct thread` to manage priority donation.
...
struct thread
  {
    ...
    struct list donations;        /* List of donations to handle multiple donation */
...
- Manage `ready_list` sorted
  One effective way is to make `ready_list` sorted when a thread is pushed to `ready_list` in sorted manner. This can be
  done by using pre-implemented function `list_insert_ordered()`. This function will insert a thread to `ready_list` in
  sorted order.

- Acquiring lock
  When a thread is trying to acquire a lock, following steps are done.

```
function lock_acquire(struct lock *lock)
    check if lock is already acquired by other thread
    if yes,
        add lock to thread's waiting list
        add threads to donation list
        donate priority to the holder of the lock
    
    set thread's waiting lock to nil
    acquire the lock with sema_down
         
```

- Donating priority
  When a thread donates its priority to another thread, following steps are done.
  The recursive loop to get into depth will resolve the nested donation issue.

```
function donate_priority(struct thread *donor, struct thread *donee)
    donee := donee
   loop 
...
- Releasing lock
  When a thread releases a lock, following steps are done.

```
function lock_release(struct lock *lock)
    remove lock from thread's waiting list
    remove thread from donation list
    refresh thread's priority based on the highest priority in donation list
    release the lock
```

- Change of priority in running thread
  When a thread's priority is changed, following steps are done.

```c
function thread_set_priority(int new_priority)
    if current thread has no donation, 
      change priority to new_priority
    else 
      change original_priority to new_priority
    
    if new_priority is lower than current priority
        if next_thread_to_run's priority is higher than current priority
            yield the cpu
```

### Implementation

Implementation of key functions follows

- `ready_list` management
...
void donate_priority(struct thread *donor, struct thread *donee) {
  // TODO: dealing with depth of nested search
  while (donee->waiting_lock != NULL) { 
    donee->priority = donor->priority;
...
Above algorithm and implmentation is based on the assumetion that

1. `ready_list` is sorted in ascending order of priority
2. `waiters` in `semapahore` is sorted in ascending order of priority of thread

One implementation is mange list as always sorted. The other possible implementation is to pop element in list in
sorted manner, while unmannged within list. However, this will require more overhead to manage the list. Both
implementation will have same time complexity (inserting in sorted list -> O(n), popping in sorted manner -> O(n)).
To leverage the pre-implemented function in `list`, like `list_insert_ordered()`, we choose to manage list in sorted
manner.
