...
When dealing with the synchronization problems, it is important to assure that the preemption is not occur in the middle executing the atomic function.
So, it is necessary to prevent the preemption occurs.
In pintos, the preemption occurs when the `thread_ticks` get greater than `TIME_SLICE`.
The `thread_ticks` is increased by the `thread_tick()` which is called by the external interrupt handler `timer_interrupt()`.
Thus, disabling the interrupt can be the solution.
The interrupt can be controlled manually via the functions defined in `src/interrupt.c`.
Specifically, disabling interrupts can be done by `intr_disable()` and can rollback interrupts by `intr_set_level()`.
...
In general, according to the definition of the semaphore, the value of the semaphore should be initialized to a positive number depending on the number of resources available.
...
But they don't deal with how to control the order in the interdependent threads.
Condition variable defines how the dependent thread give the control flow to the responsible thread and take it back.

If a thread(Thread A) want to access the common resource after the another thread(Thread B) work on that common value, it uses condition variable.
In this case, the "condition" is "Is Thread B finished its work?". The Thread A waits for the condition become true, and the Thread B sends signal to Thread A if the condition satisfied. 
Here is the brief explanation of how condition variable works using a lock and a semaphore.

- First, the Thread A needs to lock the common resource and acquire to prevent other threads to access.
- Then it prepares a semaphore with initial value 0 to block itself until the common resource ready.
- After preparing the semaphore, it releases the lock so that the Thread B can access and blocks itself using the prepared semaphore.
- The Thread B acquires the lock and does the work needed on the common resource.
- When the job done, it sends a signal to the blocked thread, Thread A, which is the V operation of the semaphore.
- Finally, it can release the lock for Thread A.

In pintos, a condition variable is implemented by a struct with a single variable.
```c++
struct condition
  {
    struct list waiters;    /* List of waiting threads */
...
The list `waiters` holds the elements of the struct `semaphore_elem`. 
...
- `cond_init()`: Simply initialize the list `waiters`.
...
After calling `cond_wait()`, the `waiter` semaphore initialized with the `value` 0 and `list_push_back()`ed into `waiters`. 
Then the `lock` released and the `waiter` semaphore downed to make the thread wait for "signal".
As the `lock` released, it can be acquired by other threads. The thread which acquire the `lock` will send the signal to the waiting thread.
The "signal" will make the semaphore up, and after receiving the signal, the thread re-acquire the lock.
- `cond_signal()`: Called by the current thread which acquires the lock released in `cond_wait()`.
Simply makes the first waiting semaphore up (i.e. make the waiting thread be in the ready state).
...
An optimization barrier is the barrier to indicate compiler not to optimize or reorder across it.
Without the barrier, the compiler is free to optimize and to reorder the instructions and this would result the buggy program.
Because the order of the instructions do an important role in the synchronization problem.
