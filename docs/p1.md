...
Also we need to append two new members to the thread structures, `nice` and `recent_cpu`.
...
The `recent_cpu` is calculated with "Exponentially weighted moving average".

We need a another new variable `load_average` to calculate the `recent_cpu`.
...
Calculating the `recent_cpu`

- `recent_cpu` = (2 * `load_avg`) / (2 * `load_avg` + 1) * `recent_cpu` + `nice`

Calculating the `load_avg`
