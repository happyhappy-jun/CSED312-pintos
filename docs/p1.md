...
thread
has used up its time slice, `thread_tick()` (called by `timer_interrupt`) will call the
...
- `lock_acquire()`: Checks whether the `holder` is the caller(it must not), and then invoke `sema_down()`. Then, set
  the `holder` to the current thread.
...
```c
// Add this function to where two conditions occurs, change or priority in running thread. new thread added to ready list
...
Also, we need a new global variable `load_average` to calculate the `recent_cpu`.
...
- As the kernel of the pintos doesn't support the float point calculations, we need fixed point calculations to deal
  ...
  We need some revise to get a correct calculation using fixed point representation, and each equation is in the
