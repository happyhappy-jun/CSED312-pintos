...
Disable `thread_get_priority()` and `thread_set_priority()` if the `thread_mlfqs` is set.
...
/** Note 
 * Returns not exact value of the fixed point `recent_cpu`, 
 * but `recent_cpu` multiplied with 100 rounded to nearest integer. 
**/
...
/** Note: 
 * Returns not the exact value of the fixed point `load_avg`, 
 * but `load_avg` multiplied with 100 rounded to nearest integer 
**/
