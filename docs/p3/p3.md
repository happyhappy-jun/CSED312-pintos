...
page가 allocate되면 frame이 할당된다. frame은 page에 대응하는 메모리 공간으로 실제 물리 주소 공간에 존재한다.

유저 프로세스에서 가상 주소를 실제 물리 주소로 변환하는 것은 Page directory와 page table을 통해 이루어진다.
유제 프로세스의 가상 주소를 실제 물리 주소로 변환한다는 것은 page에 대응하는 frame을 찾는 것을 의미한다.
...
PTE를 통해 page의 physical memory에 존재하는 실제 주소, 즉 frame을 찾을 수 있다.
