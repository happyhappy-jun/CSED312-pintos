...
따라서 evict 하기 전에 target 소유자의 supplemental page table에 target의 정보를 업데이트 해줘야 한다.
...
struct spt_entry *spt_add_entry_swap(struct hash *spt, void *vaddr) {
  ASSERT(spt != NULL)
  struct spt_entry *old;
  struct spt_entry *spte = malloc(sizeof(struct spt_entry));
  spte->vaddr = pg_round_down(vaddr);
...
load_page()가 성공한다면 pf_restart()를 호출하여 intr_frame을 수정하여, page fault를 발생시킨 명령어를 재실행하도록 한다.

load_page()는 인자로 받는 spte의 성격에 따라 다른 방법으로 load on demand를 수행한다.
...
install_page()까지 성공적으로 진행되었다면, spt_add_entry_stack()을 통해 supplemental page table에 새로 할당된 stack page를 추가한다.
stack growth가 성공적으로 이루어진 경우에는 true를 반환하고, 그렇지 않은 경우에는 false를 반환한다.
